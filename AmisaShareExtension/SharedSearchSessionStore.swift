...
//
//  Même clé et logique que l’app native (`Balibu/Services/SharedSearchSessionStore.swift`).
//
...
final class SharedSearchSessionStore {
    ...
    static let appGroupIdentifier = ShareExtensionStorage.appGroupIdentifier
    private static let sessionResultsDirectory = "SessionResults"
    private static let continuityDirectory = "ContinuitySnapshots"
    private let storageKey = "amisa.pendingSharedSearchSession.v1"
    ...
    static func continuitySnapshotURL(fileName: String) -> URL? {
        guard let base = FileManager.default.containerURL(forSecurityApplicationGroupIdentifier: appGroupIdentifier) else {
            return nil
        }
    ...
    func updateContinuitySnapshotFileName(_ fileName: String?) {
        guard var s = peekPending() else { return }
        s.continuitySnapshotFileName = fileName
        try? save(s)
    }
    ...
    func clear() {
        if let pending = peekPending() {
            if let name = pending.completedResultJSONFileName,
               let url = Self.sessionResultJSONURL(fileName: name) {
                try? FileManager.default.removeItem(at: url)
            }
            if let name = pending.continuitySnapshotFileName,
               let url = Self.continuitySnapshotURL(fileName: name) {
                try? FileManager.default.removeItem(at: url)
            }
        }
