...
struct SharedImportReviewView: View {
    ...
    private var editingView: some View {
        GeometryReader { geo in
            ZStack(alignment: .top) {
                ...
                // Contenu vertical
                VStack(spacing: 0) {
                    // Espace réservé pour le header : safeTop + 16 (padding) + 50 (bouton) + 20 (respiration)
                    Color.clear
                        .frame(height: geo.safeAreaInsets.top + 86)
    ...
    private func bottomPanel(geo: GeometryProxy) -> some View {
        VStack(spacing: 0) {
            ...
            Text(String(localized: "Déplace et pince pour recadrer"))
                .font(.system(size: 15))
