...
struct MainTabContainerView: View {
    ...
    var body: some View {
        ZStack(alignment: .bottom) {
            ...
            if !router.isTabBarHidden {
                // Gradient de lisibilité derrière la tab bar (ne bloque pas les taps)
                tabBarReadabilityGradient

                BottomNavigationRow(router: router, onScan: { router.openPhotoAnalysis() })
            ...
            NavigationStack(path: $router.path) {
                SearchHomeView(searchHistoryService: .shared)
                    ...
                    .navigationDestination(for: AppRoute.self) { route in
                        ...
                        case .manualSearchLoading(let query):
                            ManualSearchLoadingView(query: query)
                        case .results(let session):
                            ResultsView(session: session)
                        case .searchHistory:
                            SearchHistoryView()
                        case .remoteSessionLoading(let sessionId, let continuitySeed):
                            RemoteSessionLoadingView(sessionId: sessionId, continuitySeed: continuitySeed)
    ...
    /// Gradient adaptatif light/dark derrière la tab bar — ne bloque jamais les taps.
    private var tabBarReadabilityGradient: some View {
        LinearGradient(
            colors: [
                (colorScheme == .dark ? Color.black : Color.white).opacity(0.75),
                .clear
            ],
            startPoint: .bottom,
            endPoint: .top
