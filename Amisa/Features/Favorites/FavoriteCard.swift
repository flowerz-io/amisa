...
struct FavoriteCard: View {
    ...
    private var favoriteImageLayer: some View {
        if favorite.mode == .textQuery, !favorite.previewImageURLs.isEmpty {
            ManualSearchPreviewCollage(imageURLs: favorite.previewImageURLs, cornerRadius: 18)
                .frame(width: cardWidth, height: imageHeight)
        } else if let image = favoriteImage {
            image
                .resizable()
                .scaledToFill()
                .frame(width: cardWidth, height: imageHeight)
