//
//  SquareCropEditor.swift
//  Balibu
//
//  Cadre carré fixe : pan/zoom derrière le cadre ; export = zone visible (une seule source de vérité).
//
...
struct SquareCropEditorRepresentable: UIViewControllerRepresentable {
    let image: UIImage
    var onControllerReady: (SquareCropEditorViewController) -> Void

    func makeCoordinator() -> Coordinator {
        Coordinator(onReady: onControllerReady)
    }

    func makeUIViewController(context: Context) -> SquareCropEditorViewController {
        let vc = SquareCropEditorViewController(image: image)
    ...
    func updateUIViewController(_ uiViewController: SquareCropEditorViewController, context: Context) {}

    final class Coordinator {
        let onReady: (SquareCropEditorViewController) -> Void
        init(onReady: @escaping (SquareCropEditorViewController) -> Void) {
...
final class SquareCropEditorViewController: UIViewController {
    ...
    /// Export strictement la zone visible du cadre carré (scrollView bounds → pixels image).
...
extension SquareCropEditorViewController: UIScrollViewDelegate {
