...
final class SharedSearchSessionStore {
    ...
    func clear() {
        if let pending = peekPending() {
            ...
            removeContinuitySnapshotIfNeeded(pending.continuitySnapshotFileName)
        }
    ...
    func removeContinuitySnapshotIfNeeded(_ fileName: String?) {
        guard let name = fileName, let url = Self.continuitySnapshotURL(fileName: name) else { return }
        try? FileManager.default.removeItem(at: url)
    }
}
